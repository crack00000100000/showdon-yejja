# Quick-Reference 체크리스트 (★ v1.9.1 NEW — 작성 시 항상 참조)
...
## 모드 B-1 Preview 작성 — 출력 직전 체크 (★ v1.9.3 hard rule 8개 + soft 권장)
...
- [ ] **§6.2b OCR 우선** — STT 와 OCR 다르면 OCR 채택 (영상이 ground truth)
- [ ] **§13.5 통합 sync 재검증 6 항목** (★ v1.9.3 — Preview 단계 + 저장 직전) — timeline align / frame OCR / 통독 시뮬 / STT 비교 / explain 컨텍스트 / **★ explain fact 근거 명시**
...
- `ocr_local.json` (옵션) — PaddleOCR cross-check
...
| **6 ★ NEW v1.9.3** | **explain 라인 fact 근거 명시 (§9.6 핵심 룰)** | 모든 explain 라인이 `(구체 fact) (평가/감상)` 구조인가. §9-pre 영상 흐름 표의 "구체 디테일" 컬럼에서 fact 추출 검증 | 단어 1~2개 평가 (`(미친)`, `(빌드업 ㅋㅋ)`) 0건. fact 근거 누락 라인 0건. |
...
- **6 미달** ★ NEW → 해당 explain 라인 — §9-pre 영상 흐름 표의 fact 추가하여 `(fact) (평가)` 구조로 재작성. 매칭 fact 없으면 explain 라인 자체 삭제.
...
## §9.6 — explain 카피 — fact 근거 의무 + 도입부 5종 + 시점별 톤 (★ v1.9.3 전면 강화)

> **★ v1.9.3 정체성 — 카피의 "왜" 명시 의무**
>
> v1.9.2 결과물 5개 분석 — explain 이 어미 (`~는데`) 와 키워드 (`미친`, `압권`) 만 박고 *fact 근거* 비어있음. 사용자 피드백 "재미도 없고 맥락도 못 잡음".
>
> v1.9.3: 모든 explain 라인 = **(구체 fact) (평가/감상)** 구조 의무. 단어 1~2개 평가 (`(미친)`, `(빌드업 ㅋㅋ)`) 절대 금지.
>
> 정체성 = **리뷰어 시점 큐레이터 코멘트** (★ 우선) + 편집자 narration (보조). 객관 묘사 X.

### 핵심 룰 (★ v1.9.3 hard rule)
