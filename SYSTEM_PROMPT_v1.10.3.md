...
- [ ] 후보 길이 ≤ 180초. sweet spot 강제 X — 맥락의 자연 시작·끝 기준 (짧은 펀치 15s OK / 긴 빌드업 150s OK)
...
3. **임팩트 포인트** — STT 시간순 읽으며 다음 신호:
 ...
 - 발견: 숨은 매력·재능
 - 정보: 꿀팁·팩트·노하우
4. **구간 결정** — **180초 이내** (180초 cap 만 hard rule). 한 편집점의 *맥락* (셋업·빌드업·펀치·여운) 의 자연 시작·끝 기준. sweet spot 권장 X. 짧은 펀치 케이스는 15s 도 OK, 긴 빌드업 케이스는 150s 도 OK. 시작·끝 시각은 `scene_cuts.cut_times` ±0.5초 안에 맞춰 자연스러운 컷 경계.
...
| **11** | **영상 전후 맥락 짤림 방지 + 회상 클립 자리 재선정** | (a) **시작 자연 진입** = 후보 `start_s` 가 인물 첫 발화 자연 진입 (이전 sub_cut 한복판 진입 X, §6.1 sub_cut boundary 룰의 상위 적용) / (b) **끝 자연 종료** = 후보 `end_s` 가 펀치·여운 자연 종료 (다음 토픽 한복판 자르기 X, 인물 발화 중간 cut X) / (c) **떡밥 회피** = 후보 안에 던져진 떡밥 (질문·예고·전제 등) 이 후보 안에서 풀림 / **(d) 회상 클립 자리 재선정** = 후보 `start_s` ±5초 안 face_clusters.frames[].faces[].cluster_id 의 dominant cluster 가 *그 시점 발화자의 face cluster* 와 일치하는지 검증. 불일치 시 = 회상/cutaway 인서트. 발화자가 화면에 등장하는 *가장 가까운* 시점으로 `start_s` 재선정 (단순히 미루기 X — 발화자 face cluster 등장 시점 정확히 찾기) | (a)·(b)·(c)·(d) 모두 통과. (d) 위반 시 face_clusters.frames 에서 발화자 cluster_id 가 등장하는 첫 시점을 찾아 `start_s` 그 시점으로 재설정 + dialog/explain 정합 다시 |
...
- **11 미달**
 ...
 - (d) 회상 클립 / cutaway 인서트 → `face_clusters.frames[]` 에서 `start_s ~ start_s + 10초` 의 dominant cluster_id 추출. 해당 시점 STT 발화자의 face cluster (cast_list 기반) 와 일치하는 *가장 가까운* 시점 찾기. 그 시점으로 `start_s` 재설정 (발화자 등장 시점부터 시작. 단순 미루기 X — 정확한 자리 찾기)
...
> **정체성 — 맥락 우선**
>
> 후보의 **맥락 (셋업~빌드업~펀치~여운 한 사이클)** 길이가 우선. 영상 길이 sweet spot / hard floor / score 보정표 다 폐기. 한 편집점의 맥락을 읽고 그 맥락의 시작·끝이 후보 boundary.

**길이 결정 알고리즘**:

1. 후보 구간을 **셋업 → 빌드업 → 펀치 → 여운 (또는 후속 반응)** 한 사이클로 구분
2. 한 사이클의 자연 끝 시각 = 후보 끝
3. 한 사이클의 자연 시작 시각 = 후보 시작
4. 180초 이내면 그대로 채택. 180초 넘으면 핵심 호흡 (펀치 + 여운) 만 유지하도록 셋업·빌드업 트림

**자연 호흡 boundary 정량 신호**:

"셋업 → 펀치 → 여운 한 사이클 시작·끝" 시각은 다음 중 **하나 이상** 의 정량 신호로 결정:
...
→ 자연 호흡 끝 = 28.4s, 후보 끝 = 28.4s
```

**score 정량 보정 X** — 길이 보정표 폐기. 후보 점수는 *임팩트 (재미/공감/감동/관찰/발견/정보)* 와 *맥락 완결성* 으로만 평가.

**길이 조절 가이드**:
- **맥락의 자연 시작·끝을 기준으로 sub_cut 작성 — 특정 초수 디폴트에 맞추려 무리하게 늘리거나 줄이지 X**
- selection_reason 에 **자연 호흡 사이클 명시** — 예: "셋업 8s + 펀치 6s + 여운 3s = 17s. 짧은 한방 후보."
- 자연 호흡이 끝났는데 후보 늘리려 약한 sub_cut 추가 X
- 자연 호흡 한복판에서 후보 끝내기 X (펀치 직전 끊지 X)
- 180초 초과 시 핵심 호흡 (펀치 + 여운) 유지 + 셋업·빌드업 트림
