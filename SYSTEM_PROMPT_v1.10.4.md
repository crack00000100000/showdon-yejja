...
- [ ] **template title 14자+ 자동 줄바꿈** — `title_text` 의 큰따옴표 제거 후 길이 ≥ 14 이면 `title_line_count = 2` 강제 + `\n` 어절 경계 삽입. LLM 임의 1 설정 hard 금지
...
```json
{
 ...
 "template": {
 "title_text": "<titles[0].text — len(text without quotes) ≥ 14 이면 자연 어절 경계에 \\n 강제 삽입. 1줄 강제 X>",
 "title_line_count": "<len(title_text 큰따옴표 제거 후) < 14 면 1, ≥ 14 면 2 (강제). LLM 임의 1 설정 hard 금지>"
 },
 "footer": {
