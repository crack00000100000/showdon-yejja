...
- [ ] **★ v1.9.7 §9.2 explain 라인 수 권장** (★ hard rule 17) — 영상 길이별 () explain: ~30s 5~8개 / 30~50s 7~12개 / 50~60s 10~16개. 234 영상 평균 7.7개. 무 () 영상 (dialog 강한 영상) 14.5% 허용
- [ ] **★ v1.9.7 §9 길이 — 1~5자 minimal 우선** — () explain 의 75%+ 가 1~5자 (부호 / 자음 / 명사+@ / 의태어). 11+자 긴 narration 은 5% 이하 (Family 11 변형). 1줄 8~16자 강제 룰 폐기

### Soft rule (권장 — 노력해서 조정, hard 깨면서까지 X)

- [ ] dialog/sec ≥ 3.5 (§6.2 D)
- [ ] 글자수 분포 — 5~12자 60%+ / 10~15자 20%+ / 5자 미만 30%- (§6.2 — 결과 분포)
- [ ] **★ v1.9.7 §9 Position × Family 매핑** — Intro: Family 1 (X중..) / Setup: Family 3 (의태어) / Punch: Family 4 (감정+@) + Family 6 (부호) / Outro: Family 12 (결과)
- [ ] **★ v1.9.7 §9 길이 분포** — 1~2자 (부호 only) 40%+ / 3~5자 (명사+@, 의태어) 35%+ / 6~10자 (X중, X ON) 20% / 11+자 5% 이하
- [ ] **★ v1.9.7 §9 부호 우선순위** — `..` (가장 흔함) > `;;` > `..ㄷㄷ` > `!!!!!` > `?????` > emoji
- [ ] **★ v1.9.7 §9 X ON / X OFF 비유** — 영상당 1~3회 권장 (모드 진입 announcement). 시그니처 motif: `(스프링쿨러 ON)` `(GTA ON🔥)` `(스텔스 OFF)` 등
- [ ] **★ v1.9.7 §9 Narrative arc** — Arc A (X중→X 성공) / Arc B (1트→2트→실패) / Arc C (OFF→ON) / Arc D (의태어→정서) / Arc E (시리즈 시그니처) / Arc F (Ring composition)
- [ ] **★ v1.9.7 §9 시그니처 모드 토글** — `(X ON)` 22번+ 검증된 시그니처. 영상의 모드 진입을 비유로 narration
...
5. ★ **v1.9.7 — explain 카피는 위 표의 "구체 디테일" 컬럼에서 Family 매핑** (v1.9.3 fact reference 룰 폐기):
   ...
   - **금지** (v1.9.7): `~는데` `~미친` `~압권` 등 평가 어미·평가 어휘
...
>
> v1.9.6 까지의 룰 (`~는데/~인데` 어미 35%+, 리뷰어 평가 어휘, 1줄 8~16자) 은 **234개 reference shorts 데이터 분석 결과 0개 영상** 에서 검증되지 않았음. 사용자가 v1.9.6 까지 explain 매너리즘 탈출 시도를 여러 번 했으나 모두 실패. v1.9.7 부터 **TV 예능 자막 narration 형** 으로 전면 재구성.
>
> **v1.9.7 explain.srt = 큐레이터의 TV 예능 자막 narration**. 객관 narration + 명사형 + 의성어/의태어 + 모드 토글 비유 + @ 검열 + 시그니처 caption library 재사용 + Hold 강조.
>
> v1.8/v1.9.x 부분 룰 유지: dialog 가 충분히 강하면 explain 생략 OK (234 영상 중 34개 = 14.5% 가 () 0개).

---

#### 9.1 정체성 — TV 예능 자막 narration 형 ★ v1.9.7 NEW
...
| 영상 길이 | 평균 실제 () | v1.9.7 권장 () | 비고 |
|---|---|---|---|
| ~30s 짧음 | 5~8 | **5~8** | 단일 클립 골드마인 가능 |
| 30~50s 표준 | 7~10 | **7~12** | 표준 골드마인 zone (대다수 영상) |
| 50~60s 풀 | 10~14 | **10~16** | compilation 풀 capacity |

**무 () 영상 (14.5%)**: dialog 만으로 충분히 강할 때 () 0개 OK — v1.8 룰 유지.
...
**★ v1.9.7 NEW — Hold 룰 (1792 () 중 492회 paren_hold 발견)**:
...
#### 9.4 카피 룰 — 12 Family 시스템 (★ v1.9.7 신규 — v1.9.6 카테고리 전면 폐기)
...
같은 caption 2~4 frames 유지. v1.9.7 의 narrative 강조 핵심.
...
#### 9.5 시그니처 caption library (재사용 motif 풀) ★★★ v1.9.7 핵심

234 영상 중 ≥ 3회 등장한 caption (총 약 130개). **영상 간 자유롭게 재사용 가능** — 큐레이터의 정체성 풀.
...
#### 9.6 Position × Family 매핑 — 시점별 가이드 (★ v1.9.7 NEW)
...
#### 9.7 길이 / 부호 micro-rule (★ v1.9.7 NEW)
...
#### 9.8 Narrative arc 패턴 (★ v1.9.7 NEW)
...
#### 9.9 carbon 카피 비교 예시 (v1.9.6 → v1.9.7 변환)
...
#### 9.10 sub_cut ↔ () explain 매칭 (★ v1.9.7 갱신)
...
#### 9.12 v1.9.6 vs v1.9.7 핵심 차이 표

| Dimension | v1.9.6 | v1.9.7 |
|---|---|---|
...
234 영상 중 인물 호칭이 () 안에 들어간 사례 거의 없음. v1.9.7 부터 **인물 호칭 X**.

대신 **인물군 추상 명사화** 허용 (Family 11 'X의 Y'):
- `(2025년의 아빠와 아들)` 5x ★
- `(K-지하철 빌런)` `(행인 1)` `(어머니 등@장)` etc.

#### 9.6 톤별 빈도 — v1.9.7 폐지 (§9.2 빈도 표 + §9.6 Position × Family 로 대체)

v1.9.6 의 톤별 5~12% 등장 빈도 룰은 v1.9.7 §9.2 빈도 표 + §9.6 Position × Family 매핑으로 통째 교체.
