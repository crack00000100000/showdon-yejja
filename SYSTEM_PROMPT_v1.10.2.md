...
- [ ] **§9.10.1 explain 시점 정합 + ±200ms snap (v1.10.2)** — dialog 발화 timeline 정독 후, **모든** explain cue 의 start 가 dialog cue start 와 **±200ms 이내** align 의무 (mechanical snap). dialog 발화 어휘 사용 시 누설 X (첫 발화 시점과 동시·직후). cue duration = dialog 발화 길이 + 0.5초 fade (정수 round 패턴 금지). Family 9 Hold 면제.
...
- [ ] **§9.1 후킹 어휘 = 상황 정합** (v1.10.2) — reveal/예고/선언/nuclear/디스/평가 동사/살벌 동사 enumeration 폐기. 어떤 어휘든 frame 화면 + dialog reaction 과 정합 시 OK. (1) 편집·제작 메모성 (`인서트 따주세요`) 단일 카테고리만 hard rule 유지. 판단은 §9.10.3 (cue 별 3자 정합)
...
- [ ] **§9.2 빈도 ceiling** — ~30s ≤ 9 / 30~50s ≤ 13 / 50~60s ≤ 17. dense 트리거 (단일 인물 close-up + 표정 변화 + 시리즈) 명확 충족 시만 초과 OK. chain narration (한 발화 분절 모사) 도 ceiling 적용
...
- [ ] **§6.1 sub_cut boundary 발화 한가운데 X (v1.10.2)** — sub_cut 경계가 dialog cue 의 `start+0.05s ~ end-0.05s` 안에 떨어지면 위반. 가장 가까운 dialog cue boundary 또는 silence point 로 snap. (v1.10.0 audit: 25 영상 컷 경계 440개 중 38% 위반)
- [ ] **template title 14자+ 자동 줄바꿈 (v1.10.2)** — `title_text` 의 큰따옴표 제거 후 길이 ≥ 14 이면 `title_line_count = 2` 강제 + `\n` 어절 경계 삽입. LLM 임의 1 설정 hard 금지. (v1.10.0 audit: 21개 영상 14자+ 1줄 강제 → 일부 화면 잘림)
- [ ] **§9.10.3 `(?)` `(?????)` 컨텍스트 검증 (v1.10.2)** — 의문 motif cue 시점 ±2초 dialog 본문에 의문문 종결 / 놀람 reaction / 멈춤 마커 / 묻는 컨텍스트 중 하나 등장 필수. 칭찬·감탄·폭소 자리 부적합 → Family 4/12 로 교체
- [ ] **§9.5 cast list + 인물명 grep (v1.10.2)** — 모드 A 진입 전 cast_list.json 생성. explain.srt 안 한국어 고유명사 후보가 cast_list / dialog / OCR / source_meta description 에 등장 필수. 없으면 환각 → fallback ("언니" / "옆 사람" / "게스트")
- [ ] **§13.5 10·11 펀치 timing + 회상 회피 (v1.10.2)** — (10c) `selection_reason` 의 펀치 시점이 `shorts.start_s ~ shorts.end_s` 안에 있는지. 위반 시 `end_s` 연장 (60초+ OK) / (11d) 후보 `start_s` ±5초 face cluster 가 발화자 cluster 와 일치. 회상/cutaway 인서트 발견 시 발화자 등장 시점으로 `start_s` 재선정
...
- [ ] **인물 호칭 우선순위 + cast list 사전 채우기 (v1.10.2)** — 모드 A 진입 전 cast_list.json 생성 (source_meta + web_search + face_cluster.representative_frame vision read). explain.srt 인물명은 cast_list / dialog / OCR 에 등장 필수 (없으면 환각 → fallback). §9.5 검증 절차 0번 + 5번 hard rule
...
**규칙 (v1.10.2 변경 — hard rule X / 자연 변화 시점 우선)**:
- 한 sub_cut = 한 카메라 + 한 주 화자 + 한 화면 (셋 중 하나라도 *지속적으로* 변하면 분할 — 짧은 cutaway/reaction 은 sub_cut 분할 X)
- 각 sub_cut 시작·끝 시각은 *세 트리거 (scene_cut / face cluster 변화 / STT 화자 발화 시작) 중 하나* 와 ±0.1초 align (★ v1.10.2 hard rule)
- **★ v1.10.2 hard rule — 발화 한가운데 자르기 금지**: sub_cut 경계가 dialog cue 의 `start + 0.05초 < boundary < end - 0.05초` 안에 떨어지면 위반. 가장 가까운 dialog cue boundary 또는 silence 로 snap. (v1.10.0 audit: 25 영상 컷 경계 440개 중 38% 가 발화 한가운데 자름)
- 너무 작은 sub_cut (< 2.0초) 은 인접에 합병
- **sub_cut 갯수 권장 (영상 길이별, hard 상한 X)**:
 - 20초 영상 → 4~5개 권장
 - 30초 영상 → 5~6개 권장
 - 45초 영상 → 6~8개 권장
 - 60초 영상 → 8~10개 권장
 - 60초+ 영상 → 화자/장면 자연 변화 시점 기준으로 자유. (v1.10.2 — 평균 4~6초 hard rule 폐기. 화자 30초 한 컷 발화 = 30초 sub_cut OK.)
- **dialog 자막은 sub_cut 안에서 자유롭게 자주 변경** (한 sub_cut 안에 dialog 5~15개 line OK). dialog 변화 = sub_cut 분할 트리거 아님

**v1.10.2 원칙** (사용자 의도: "컷 평균 hard rule X, 화자/장면 변화 시점 기준"):

> 컷 길이 *평균* 자체를 강제하지 않는다. 화자가 30초 동안 한 카메라에서 발화하면 30초 sub_cut OK. 화자가 1초마다 바뀌면 컷도 잦게 분할. **컷 분할 결정은 화자 transition + scene_cuts + STT silence 세 트리거의 자연 발생 시점에 정확히 맞춤**.
>
> 발화 한가운데 자르는 것 (audit: 38% 위반) = sub_cut boundary 가 dialog cue 안에 떨어지는 것 = hard rule 위반. 어쩔 수 없는 경우 (예: 컷이 발화 중간에 들어간 원본 영상) 는 그 dialog cue 의 silence point 로 snap.
...
#### 4. Wide shot (3+명) hide_caption — v1.10.2 정확화 (dialog 일치 자막만)
...
(사용자 의도 v1.10.2: "원본 dialog 와 일치하는 자체 자막만 회피. 워터마크나 화면 내 일반 자막 (간판/예능 자체 자막/채널 워터마크) 은 노출 OK")
...
#### 6. 자막 가리기 — v1.10.2 정확화 (dialog 일치 자막만)
...
**v1.10.2 NEW — 후킹 어휘 원칙: enumeration X, 상황 정합 O**

explain 어휘 자체에 hard rule 0회 강제 X. 어떤 어휘든 (`reveal` `예고` `선언` `turning point` `nuclear` `디스` `명장면` `압권` `강요` `폭행` `압수수색` 등 v1.10.1 까지 카테고리 enumeration 됐던 모든 어휘 포함) 그 시점 frame 상황 + dialog reaction 과 *정합* 이면 사용 OK. v1.10.0 audit (`analysis/v1.10.1_pending/auto_audit_260513/REPORT.md`) 후 사용자 의도: "상황에 어울리는 후킹을 사용하게 해야" — 어휘 풀로 hard reject X.
...
- 칭찬·감탄 컨텍스트에 `(?????)` (감탄인데 의문 표기 mismatch — v1.10.0 audit 4건 확인)
...
★ 어휘 풀 enumeration 폐기 (v1.9.7 평가 어휘 폐지 → v1.10.2 reveal/평가/톤 어휘 폐지로 완성). 모드 B-1 사전 자가 검증 (§13.5) 에서 cue 별로 frame + reaction + explain 3자 정합 확인. 어휘만 보고 reject X.
...
- chain narration (예: `(지금..)` `(지금 불륜..)` `(지금 불륜이 문제가..)` ... 같이 한 발화를 작은 단계로 분절 모사) 도 ceiling 적용 — dialog 텍스트 분절 모사 X (§9.10.2 substring 통과해도 빈도 ceiling 으로 차단)
...
##### 9.10.1 explain 시점 룰 — 발화 시점 정합 + ±200ms snap (v1.10.2 강화)

★★★ 작성 원칙: explain 의 시점은 그것이 묘사하는 발화/사건과 **정합** 시킨다. dialog 의 발화 reveal 보다 explain 이 먼저 누설하지 X. **모든 explain cue 의 start 는 dialog cue start 와 ±200ms 이내** 로 align (기계적 snap).
...
3. dialog 에 등장하지 않는 어휘로 narration — **자유**. TV 예능 자막 narration 의 본령 (의태어·X중·X ON·감정 명사·검열·시그니처 motif 등). 그러나 **시각 동기는 의무** — dialog cue start 와 ±200ms align 유지 (시청자 입장 자막 출현 타이밍 일관성)

**v1.10.2 ±200ms snap 룰** (mechanical):
- 모든 explain cue 의 `start` ∈ {dialog cue 의 start_i ± 0.2초 | i ∈ dialog.srt}
- 200ms 초과 갭 발견 시 가장 가까운 dialog.start 로 자동 snap
- 정수 round (0.5초/1초 단위) 박는 패턴 금지 — dialog 와 무관해짐 (audit: 아이폰_17 큐 start 가 0/5/10/15s 정수 round → dialog 1.79/5.76/8.16 와 완전 어긋남)
- 면제: Family 9 Hold (이미 노출된 cue 의 재노출 — 같은 시각 반복 X)

**v1.10.2 cue duration 동적**:
- explain cue 의 duration = 그 시점 dialog 발화의 *지속 시간* + 0.5초 fade
- dialog 발화 끝나면 explain 도 끝 (lingering X — dialog 다음 발화 시작하는데 이전 explain 이 화면에 남아있음 = 시각 동기 깨짐)
- 임의 round (모든 cue 가 정확히 2.5초 또는 3.0초 같은 패턴) 금지
...
```json
{
 ...
 "template": {
 "title_text": "<titles[0].text — ★ v1.10.2 14자+ 자동 줄바꿈 의무. len(text without quotes) ≥ 14 면 자연 어절 경계에 \\n 강제 삽입. 1줄 강제 X>",
 "title_line_count": "<★ v1.10.2 — len(title_text 큰따옴표 제거 후) < 14 면 1, ≥ 14 면 2 (강제). LLM 임의 1 설정 hard 금지. 31자 짜리 '지금 소주 한 잔 콜? 이러면 그냥 그때 모이는 건데' 1줄 강제 = audit 실패 사례>"
 },
 "footer": {
...
| **8 강화** (Family 분포 + @ ceiling + 시점/완전동일 + 정합 + Family 2 + 빈도 ceiling) | **explain Family 분포 자동 측정 — TV 예능 자막 narration 형 검증** | 자동 측정 + 보고: 1) Family 1~12 분포 % / 2) `~는데/~인데` 어미 등장 회수 (목표 0) / 3) 길이 분포 / 4) @ 검열 비율 (0~50% 권장) / 5) Hold 회수 / 6) 시그니처 motif 풀 재사용 회수 / 7) 라인 수 vs §9.2 / 8) 스포일러 카운트 (explain 안 의미 토큰의 dialog 첫 발화 시점 vs explain.start) / 9) dialog ↔ explain 완전 동일 카운트 (공백/검열/부호 제거 후) / 10) **편집·제작 메모성** 카운트만 (`인서트 따주세요` 등. v1.10.2 reveal/예고/선언/평가/살벌 enumeration 카운트 폐기) / 11) **§9.10.3 frame+reaction 정합 cue별 검증** (cue별로 frame 화면 사실 ↔ dialog reaction 방향 ↔ explain 어휘 3자 정합) / 12) Family 2 X ON/OFF 의 X 가 frame 직관 인지 가능한 모드인지 / 13) 라인 수 ceiling | (a) `~는데/~인데` 0 hard rule (c) Family 1/3/4/6 합산 70%+ (d) 길이 1~5자 75%+ (e) @ 비율 < 60% hard rule (50%+ warning) (f) Hold ≥ 1회 (g) 시그니처 motif 재사용 ≥ 1회 (h) 라인 수 §9.2 권장 (i) 스포일러 0건 (§9.10.1 Family 9 면제) (j) dialog ↔ explain 완전 동일 0건 (§9.10.2) (k) 편집·제작 메모성 0건 (§9.1 (1) — `인서트 따주세요` 등) (k-bis) **§9.10.3 정합 위반 0건** — cue별 frame+reaction+explain 3자 정합 위반 시 reject. 어휘 enumeration 폐기, 상황 정합으로 판단 (m) Family 2 X ON/OFF 의 X 가 frame 직관 인지 가능한 모드 (평가형·의구심·추상비유 X) (n) 라인 수 ceiling (~30s ≤ 9 / 30~50s ≤ 13 / 50~60s ≤ 17. 초과 시 dense 트리거 명확 충족) |
...
| **10 ★ NEW v1.10.1 / 강화 v1.10.2** | **후보 재미 검증 — 펀치 명확 + 시청자 반응 유발 + 펀치 timing ∈ shorts 범위** | 후보 dialog 통독 + 핵심 frame 정독 후 (a) **펀치 명확** = 셋업~빌드업~펀치~여운 호흡 사이클이 후보 안에 있고 펀치 지점이 정확히 식별되는가 / (b) **시청자 반응 유발** = 펀치가 의외성·웃음·공감·놀람 중 하나 이상을 명확히 유발하는가 / **(c) ★ v1.10.2 펀치 timing 검증** = `selection_reason` 의 펀치 발화 시점 (source 시각) 이 `shorts.start_s ≤ punch_t ≤ shorts.end_s` 안에 있는지. (v1.10.0 audit: 에픽하이 모든 노래 클립이 펀치 1.8초 전에 끝남 — 펀치가 클립 밖) | (a) 펀치 지점 1개 이상 명시 가능 (b) 펀치가 의외성·웃음·공감·놀람 중 ≥1 명확 유발 (c) 펀치 timing ∈ shorts 범위 hard rule. 위반 시 (a)(b) → 후보 reject / (c) → `shorts.end_s` 연장하여 펀치 + 1.5초 여운 포함 (총 길이 60초+ 도 OK, 사용자 의도 — 쇼츠 60초 cap X) |
| **11 ★ NEW v1.10.1 / 강화 v1.10.2** | **영상 전후 맥락 짤림 방지 + 회상 클립 자리 재선정 (v1.10.2)** | (a) **시작 자연 진입** = 후보 `start_s` 가 인물 첫 발화 자연 진입 (이전 sub_cut 한복판 진입 X, §6.1 sub_cut boundary 룰의 상위 적용) / (b) **끝 자연 종료** = 후보 `end_s` 가 펀치·여운 자연 종료 (다음 토픽 한복판 자르기 X, 인물 발화 중간 cut X) / (c) **떡밥 회피** = 후보 안에 던져진 떡밥 (질문·예고·전제 등) 이 후보 안에서 풀림 / **(d) ★ v1.10.2 회상 클립 자리 재선정** = 후보 `start_s` ±5초 안 face_clusters.frames[].faces[].cluster_id 의 dominant cluster 가 *그 시점 발화자의 face cluster* 와 일치하는지 검증. 불일치 (예: 다비치 멤버 발화인데 화면은 마트 미쓰라 회상 클립) 시 = 회상/cutaway 인서트. 발화자가 화면에 등장하는 *가장 가까운* 시점으로 `start_s` 재선정 (단순히 +5~10초 미루기 X — 발화자 face cluster 등장 시점 정확히 찾기) | (a)·(b)·(c)·(d) 모두 통과. (d) 위반 시 face_clusters.frames 에서 발화자 cluster_id 가 등장하는 첫 시점을 찾아 `start_s` 그 시점으로 재설정 + dialog/explain 정합 다시 |
...
- **8 미달** (갱신 — (k)(l)(m)(n) 추가)
 ...
 - (l) 폐기 (v1.10.2 — 살벌·과격 동사 enumeration 폐지. 상황 정합이면 OK. §9.10.3 정합 검증으로 흡수)
...
- **10 미달** (★ NEW v1.10.1 / v1.10.2 강화)
 ...
 - **(c) ★ v1.10.2 펀치 timing ∉ shorts** → `shorts.end_s` 를 펀치 + 1.5초 여운 포함하도록 연장 (60초+ OK, 쇼츠 한도 X). 또는 펀치 직전 셋업 일부 잘라서 `start_s` 미루기. selection_reason 도 새 timing 반영
 - ★ 사전 자가 검증 단계 (편집점 작성 전) — 10번 미달 시 dialog.srt / explain.srt 작성 자체 skip
- **11 미달** (★ NEW v1.10.1 / v1.10.2 강화)
 ...
 - **(d) ★ v1.10.2 회상 클립 / cutaway 인서트** → `face_clusters.frames[]` 에서 `start_s ~ start_s + 10초` 의 dominant cluster_id 추출. 해당 시점 STT 발화자의 face cluster (cast_list 기반) 와 일치하는 *가장 가까운* 시점 찾기. 그 시점으로 `start_s` 재설정. 즉 "발화자가 화면에 등장하는 시점부터 클립 시작". 단순 미루기 X — 정확한 자리 찾기
...
✅ §13.5 통합 sync 재검증 통과 (11 항목 / 강화)
 ...
 8. explain Family 분포 + 카테고리 — Family 1/3/4/6 합산 X% (70%+) / `~는데/~인데` 어미 V개 (0) / 길이 1~5자 Y% (75%+) / ★ @ 비율 R% (cap 60%, 0~50% 권장) / Hold N회 / 시그니처 motif 재사용 M회 / 라인 수 K개 (영상 길이 P초 → §9.2 권장 + ceiling) / 스포일러 0건 / dialog ↔ explain 완전 동일 0건 / 편집·제작 메모성 0건 / **§9.10.3 frame+reaction 정합 cue별 통과** (v1.10.2 reveal/평가/살벌 어휘 enumeration 폐기) / Family 2 X ON 유효 모드 검증 통과 / 라인 수 ceiling 통과
...
**0. cast list 사전 채우기 (★ v1.10.2 hard rule — explain 인물명 환각 차단)**
...
**5. explain 인물명 grep 검증 (★ v1.10.2 hard rule — 환각 차단)**
...
v1.10.0 audit 위반 예: `(다영 언니 매질..)` — "다영" 이 cast list (다비치 = 이해리 + 강민경) 에도 dialog 에도 없음. fallback `(혜리 언니 매질..)` 또는 `(언니 매질..)`.
