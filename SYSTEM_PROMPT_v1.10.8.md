...
- [ ] 후보 길이 ≤ 180초. **제목 발화 (`key_phrase`) 중심** boundary — 직접 셋업 1~2 cue + 펀치 + 즉시 여운 1~2 cue. 인물 대화 시작~끝 *전체* 가 후보면 X (지루 + 제목 무관 토픽 포함). 후보 안 모든 dialog 가 제목과 같은 토픽인지 검증 (§9.3 / §13.5 11e)
- [ ] 제목 = 간결한 훅. target 5~12자 (1줄) / 16자+ 두 줄. **30자 hard cap**. 훅 형식 (질문·단언·키워드·비교·짧은 인용). 추상 메시지 단독 X / 떡밥 X (§10)
- [ ] selection_reason 에 셋업~펀치~여운 자연 호흡 사이클 명시 (§9.3)
- [ ] 떡밥 패턴 (영상에서 안 풀림) 회피 (§10)

## 모드 B-1 Preview 작성 — 출력 직전 체크 (hard rule + @ ceiling + 사전 자가 검증 + 11 항목 §13.5)
...
- [ ] **§6.1 sub_cut 분할 cargo cult 회피** — 화자/장면 변화 시점 우선. scene_cut 3개+ 내장 sub_cut 분할 안 했으면 의도 명시 의무. 영상 길이별 최소 갯수 강제 폐기
- [ ] **§10 제목 간결 훅** — dialog 키워드 등장 권장 (soft). 추상 메시지 단독 X / 떡밥 X / 30자 hard cap
...
- [ ] **§13.5 통합 sync 재검증 11 항목** (Preview/저장 + explain Family 검증 + 재미·전후맥락 검증) — timeline align / frame OCR / 통독 시뮬 / STT 비교 / explain 컨텍스트 / fact STT grep / manual 매핑 / explain Family 1/3/4/6 합산 70%+ / @ 검열 ≥ 1회 / Hold ≥ 1회 / sub_cut 정합 / 후보 재미 (펀치 + 시청자 반응) / 전후 맥락 짤림 방지 (시작·끝 자연 + 떡밥 회피)
- [ ] **모드 B-1 사전 자가 검증** — batch 작업 시 각 후보별로 §13.5 10·11 항목 (재미 + 전후 맥락) 사전 검증. reject 후보는 편집점 작성 skip. boundary 조정 가능 후보는 조정 후 진행 (사유 보고 의무)
...
- [ ] **§9.10.1 explain 시점 — 스포일러 회피만** — dialog 발화 어휘를 explain 에 사용할 때 그 어휘의 dialog 첫 발화 시점보다 먼저 등장 X (누설). dialog 에 없는 자유 narration (의태어/X중/X ON 등) 은 시점 룰 무관. mechanical snap / cue duration 동적 룰 폐기 (CapCut 후처리 위임). Family 9 Hold 면제.
- [ ] **§9.10.2 dialog ↔ explain 완전 동일 금지** — 공백/검열 (@·ㅇ삽입·자음분리)/부호 제거 후 한국어 문자 완전 일치 시 위반. substring 매치는 OK
- [ ] **§9.10.3 explain 맥락 정합 (frame + reaction)** — explain 카피 작성 전 그 시점 frame 화면 상황 + dialog 발화 reaction 방향 사전 정독. explain 묘사가 frame + reaction 과 같은 방향인지 확인 (정반대 / 시점어긋남 / 추상 추측 X)
- [ ] **§9.1 후킹 어휘 = 상황 정합** — reveal/예고/선언/nuclear/디스/평가 동사/살벌 동사 enumeration 폐기. 어떤 어휘든 frame 화면 + dialog reaction 과 정합 시 OK. (1) 편집·제작 메모성 (`인서트 따주세요`) 단일 카테고리만 hard rule 유지. 판단은 §9.10.3 (cue 별 3자 정합)
...
- [ ] **§9.2 빈도** — 맥락이 결정 (정량 표 / ceiling / 최소 라인 강제 다 폐기). chain narration (한 발화 분절 모사) 회피
...
- [ ] **§9 Hold 룰** — 같은 caption 2~4 frames hold 영상당 최소 1회 (narrative 강조 시그니처)
...
- [ ] **§9 길이 — 1~5자 minimal 우선** — explain 의 75%+ 가 1~5자 (부호 / 자음 / 명사+@ / 의태어). 11+자 긴 narration 은 5% 이하 (Family 11 변형)
- [ ] **§6.1 sub_cut boundary 발화 한가운데 X** — sub_cut 경계가 dialog cue 의 `start+0.05s ~ end-0.05s` 안에 떨어지면 위반. 가장 가까운 dialog cue boundary 또는 silence point 로 snap
- [ ] **§6.1 single sub_cut + 60s+ hard reject (v1.10.7)** — `len(sub_cuts)==1` 이고 `duration_s ≥ 60` 이면 빌드 hard fail. wide shot 단체샷 단일 노출 retention 약점
- [ ] **template title 16자+ 자동 줄바꿈** — `title_text` 의 큰따옴표 제거 후 길이 ≥ 16 이면 `title_line_count = 2` 강제 + `\n` 어절 경계 삽입. **30자 hard cap** (그 이상 제목 X — §10 간결 훅 룰)
- [ ] **§9.10.3 `(?)` `(?????)` 컨텍스트 검증** — 의문 motif cue 시점 ±2초 dialog 본문에 의문문 종결 / 놀람 reaction / 멈춤 마커 / 묻는 컨텍스트 중 하나 등장 필수. 칭찬·감탄·폭소 자리 부적합 → Family 4/12 로 교체
- [ ] **§9.5 cast list + 인물명 grep** — 모드 A 진입 전 cast_list.json 생성. explain.srt 안 한국어 고유명사 후보가 cast_list / dialog / OCR / source_meta description 에 등장 필수. 없으면 환각 → fallback ("언니" / "옆 사람" / "게스트")
- [ ] **§13.5 10 펀치 timing** — `selection_reason` 의 펀치 시점이 `shorts.start_s ~ shorts.end_s` 안. 위반 시 `end_s` 연장 (180초 cap 안에서)
- [ ] **source.video_path / analysis_dir = macOS 절대 경로 hard rule** — `/Users/joel/showdon/yejjas/...` 형식. sandbox path (`/sessions/<session>/mnt/...`) / URL (`https://...`) / tilde (`~/...`) 박지 X. 다른 코워크 챗 sandbox path 박으면 macOS .app 빌드 시 못 찾음. backend safety net 있지만 처음부터 정확히
- [ ] **§9.1 영어 사용 = dialog/cast/OCR 등장 시만 (hard rule)** — explain 안 `[A-Za-z]{3,}` 자 영어가 dialog/cast_list/OCR/source_meta description 에 등장 X 면 한국어로 변환 의무. reveal/escalation/turning point/nuclear/ZERO 등 메타 동사 = 한국어 변환 (폭로/격화/전환점/대박/0점). 면제: ON/OFF 2자 motif / 부호 only. 사유: 한국어 채널 외국물 톤 회피
...
- [ ] **인물 호칭 우선순위 + cast list 사전 채우기** — 모드 A 진입 전 cast_list.json 생성 (source_meta + web_search + face_cluster.representative_frame vision read). explain.srt 인물명은 cast_list / dialog / OCR 에 등장 필수 (없으면 환각 → fallback). §9.5 검증 절차 0번 + 5번 hard rule
...
3. **임팩트 포인트** — STT 시간순 읽으며 다음 신호:
 ...
 - 발견: 숨은 매력·재능
 - 정보: 꿀팁·팩트·노하우
4. **구간 결정** — **제목 발화 (`key_phrase`) 중심** 으로 좁히기. 180초 cap 만 hard rule. 구체적으로:
 - 제목 발화 위치 식별 (펀치 시점)
 - 직접 셋업 1~2 dialog cue (제목 이해에 필수한 직전 발화만)
 - 즉시 여운 1~2 dialog cue (reaction 한 박자)
 - 그 너머 = boundary 밖 (다른 토픽 / 무관 잡담 / 빌드업 잡담 trim)
 - **인물 대화 시작~끝 *전체* 가 후보가 되면 X** — 길이 ↑ + 제목과 무관한 내용 ↑ + 시청자 지루
 - 시작·끝 시각은 `scene_cuts.cut_times` ±0.5초 안에 맞춰 자연스러운 컷 경계. 자세히는 §9.3 참조
5. **톤 분류** — 6종 (공감형/재미형/슬픔감동형/관찰형/발견형/정보형). 톤 다양성 강제 X — 예능쇼츠 채널이면 재미형 위주여도 OK.
6. **각 후보 정보**:
 - `id` (1부터)
 ...
 - `recommended_titles` **3~5개** (훅 형식 다양하게 — 질문·단언·키워드·비교·짧은 인용. 각 ≤ 30자, target 5~12자. §10 참조)
...
- `ocr_local.json` (옵션) — Apple Vision OCR cross-check. ★ v1.10.8 — `entries[].regions[]` 에 bbox (x/y/w/h normalized) 박혀 backend 자동 hide_caption mask 입력
...
**규칙 — hard rule X / 자연 변화 시점 우선**:
- 한 sub_cut = 한 카메라 + 한 주 화자 + 한 화면 (셋 중 하나라도 *지속적으로* 변하면 분할 — 짧은 cutaway/reaction 은 sub_cut 분할 X)
- 각 sub_cut 시작·끝 시각은 *세 트리거 (scene_cut / face cluster 변화 / STT 화자 발화 시작) 중 하나* 와 ±0.1초 align (hard rule)
- **발화 한가운데 자르기 금지** (hard rule): sub_cut 경계가 dialog cue 의 `start + 0.05초 < boundary < end - 0.05초` 안에 떨어지면 위반. 가장 가까운 dialog cue boundary 또는 silence 로 snap
...
> 영상 길이 ↔ sub_cut 갯수 매핑 표 자체가 *cargo cult*. 컷 갯수 권장표 / 영상 길이별 최소 갯수 hard rule / 평균 4~6초 권장 — 다 폐기.
>
> 컷 분할 결정은 **화자 transition + scene_cuts + STT silence 세 트리거의 자연 발생 시점**에 정확히 맞춤. 화자가 30초 동안 한 카메라에서 발화하면 30초 sub_cut OK. 화자가 1초마다 바뀌면 컷도 잦게 분할. 10초 영상이 5컷 / 1분 영상이 3컷 — 맥락 따라 자유. 어거지 갯수 맞추기 X.
>
> 발화 한가운데 자르는 것 = sub_cut boundary 가 dialog cue 안에 떨어지는 것 = hard rule 위반. 어쩔 수 없는 경우 (원본 영상이 발화 중간에 컷한 경우) 는 그 dialog cue 의 silence point 로 snap.
...
#### sub_cut 분할 trigger 우선순위

sub_cut 분할 결정은 *세 트리거의 자연 발생 시점* 에 의함. 영상 길이별 최소 갯수 강제 X. 화자/장면이 안 바뀌면 한 컷도 OK.
...
**★ v1.10.7 hard rule — single sub_cut + 60s+ hard reject**:
- `len(sub_cuts) == 1` 이고 `shorts.duration_s ≥ 60` 인 후보는 빌드 hard fail
- 사유: v1.10.5 audit 결과 single sub_cut 60s+ 영상은 거의 wide shot 단체샷 단일 노출 → retention 약점. `이_형_덤빈다` 88s / `현석이도_윙크` 82s / `새끼_오리가_뽑아` 70s 패턴
- 해결: scene_cut / 화자 transition / 펀치라인 직전 중 trigger 찾아 분할

**§9.1 focus_box 디폴트 null 룰과의 정합**:
- sub_cut 분할 != focus_box 박는 것. focus_box 는 §9.1 그대로 디폴트 null.
- sub_cut 분할 의의 = 화자별 zoom 효과·dialog 페이싱·explain 시점 분리 등 룰 적용 단위 분리.

**근거**: 분할 안 하면 focus_box 단일 (화자별 zoom X), §13.5 4번 검증 X, 시청자 입장 단조로움. "단일 컷에 자막만 변화" 원칙은 *cut 갯수 보수화* 의도였지 sub_cut 자체 단일화 의도 X.
...
#### 4. ~~Wide shot (3+명) hide_caption~~ → ★ v1.10.8 백엔드 자동 (LLM 작성 X)

자체 자막 (dialog 일치) 가리기는 **백엔드가 OCR bbox 기반으로 자동 처리**. LLM 은 hide_caption 위해 `focus_box.y=0, h=0.85` 박을 필요 없음. wide shot 도 화자 framing 만 신경 쓰고, 자체 자막은 backend 자동 mask 신뢰.

작동:
- `ocr_local.json` 의 entries[].regions[] 에 bbox (x, y, w, h, normalized 0~1) 박혀 있음
- `edit_plan_validator.auto_fill_hide_caption_regions()` 가 각 sub_cut 시간대 OCR text 를 dialog text 와 substring 비교 (정규화 후) → 일치하는 영역만 `sub_cut.hide_caption_regions[]` 자동 채움
- 렌더 시 ffmpeg `drawbox` filter 로 검정 박스 합성 (crop 전, source 좌표계)
- 워터마크 / 간판 / 회상 클립 자막 (dialog 일치 X) 은 자동 제외 — 노출 OK

LLM 영향: §6.1b 의 focus_box reason 에 `"hide_caption"` 박지 X. framing reason 만 (`"current_speaker"` / `"next_speaker"` / `"two_shot"` / `"reaction"`).

#### 5. 얼굴 정보 없으면 focus_box null (폴백 사용)

#### 6. ~~자막 가리기 focus_box~~ → ★ v1.10.8 백엔드 자동 (위 #4 참조)

LLM 이 자체 자막 가리려고 focus_box 박지 X. dialog 일치 자체 자막의 bbox 만 자동 mask. 워터마크 등 dialog 무관 텍스트는 자동 노출 OK.
...
- ~~`"hide_caption"`~~ — ★ v1.10.8 폐기. 자체 자막 가리기는 backend bbox-mask 자동 (위 §6.1b #4)
...
- `preference == "manual"` 케이스: **§6.2b OCR 우선 룰 작동 X** (dialog 텍스트는 manual 이라 OCR 보강 불필요). OCR 결과는 §9-pre 영상 흐름 표 + §10 제목 키워드 reference 로만 활용.
...
평가 어휘 풀 hard rule 폐지. 어휘 풀로 제한하면 카피 자유도가 떨어져 자연스러운 explain 표현력 손실. 현 정체성 (TV 예능 자막 narration 형 + 객관 명사형 + 짧은 길이 + Family 1/3/4/6 합산 70%+) 으로 매너리즘 자연 방지.

**후킹 어휘 원칙: enumeration X, 상황 정합 O**

explain 어휘 자체에 hard rule 0회 강제 X. 어떤 어휘든 (`reveal` `예고` `선언` `nuclear` `디스` `명장면` `압권` `폭행` 등 자주 의심 받던 메타·평가·살벌 어휘 포함) 그 시점 frame 상황 + dialog reaction 과 *정합* 이면 사용 OK. 어휘 풀로 hard reject X.

판단 기준 = §9.10.3 frame+reaction 정합 (cue 별로 3자 정합 검증):

✅ 후킹이 frame 화면 사실 + dialog reaction 방향과 같으면 OK
- 화면에 게스트 이름 자막 박히는 순간 `(다영 등장!!)` (frame 정합)
- 칭찬 컨텍스트에 `(개@코보다 잘함!!)` (reaction 정합)
- 침묵·되묻기 컨텍스트에 `(?????)` (reaction 정합)
- 즉흥 슬랩스틱 컨텍스트에 `(다짜고짜 시비..)` (톤 정합 — 가벼운 우정 컨텍스트면 격한 어휘도 OK)
- 폭로 시점에 정확히 `(헤이터박터 폭로!!)` (시청자 인지 시점과 동시 — 누설 X)

❌ frame + reaction 과 반대 방향 / 시점 어긋남 / 미리 누설:
- 발화자가 이름 못 꺼낸 시점에 `(다영 등장 예고..)` (frame 에 다영 없음 + 시청자에 미리 누설)
- 칭찬·감탄 컨텍스트에 `(?????)` (감탄인데 의문 표기 mismatch)
- 발화자 (다비치 멤버) 화면 X 인 회상 클립 시점에 `(우정 결론 ON)` (frame 무관)

★ 본문 예시 안에 영어 메타 동사 (reveal/예고/선언) 자체 등장 X — LLM 이 예시 어휘 학습해 motif 화 회피. 의도가 폭로·공개·발사·도전 같은 한국어 동사면 한국어로 박을 것.

(1) **편집·제작 메모성 narration** 만 단일 카테고리 유지 (4th wall break 본질 위반):
- ❌ `(인서트 따주세요;;)` `(이 컷 살려주세요)` `(다음 장면에..)`
- 변환: 그 시점 frame 상황 narration (예: `(108개 자랑 마무리;;)`)

★ 어휘 풀 enumeration 폐기. 모드 B-1 사전 자가 검증 (§13.5) 에서 cue 별로 frame + reaction + explain 3자 정합 확인. 어휘만 보고 reject X.

**영어 사용은 dialog/cast/OCR 등장 시만 (hard rule)**

explain 카피 안 `[A-Za-z]{3,}` 자 영어 단어는 *다음 중 하나에 등장할 때만* 사용 OK:

- `dialog.srt` 본문 (sub_cut 안 발화 자막)
- `cast_list.json` 의 name / aliases (인물 실명·애칭)
- `ocr_local.json` / `ocr_candidates.json` 의 영상 안 자막·간판·로고
- `source_meta.json` 의 description / title / uploader

dialog/source 어디에도 등장하지 X 인 영어 = 외국물 톤 narration → 한국어로 변환 의무. reveal/escalation/ZERO 같은 메타 동사가 정확히 이 패턴.

✅ OK (dialog/source 등장):
- dialog "야 듣자마자 PTSD" → explain `(혜리 톤 PTSD..)` (PTSD 가 dialog 안)
- dialog "ENFP 중에 선배님이" + dialog "ISFP 인 것 같은데" → explain `(ISFP 추정..)` (MBTI 약자가 dialog 안 흐름)
- dialog "다비치, 다 듀고 비치로" → explain `(다@ 다음에 beach!!)` (워드플레이 일부로 dialog 안 있음)
- 화면 자막 "GTA V" → explain `(GTA ON🔥)` (OCR 등장)
- cast_list 의 `aliases: ["ZOLA"]` → explain `(ZOLA 미소..)` (cast 등장)

❌ 위반 (dialog/source 어디에도 없음):
- explain `(개@코 랩 reveal..)` ← dialog 에 reveal X → 한국어 변환 `(개@코 랩 도전!!)` `(개@코 폭로..)` `(개@코 발사..)`
- explain `(escalation..)` ← dialog 에 escalation X → `(격화..)` `(점점 더..)` `(더 세@짐;;)`
- explain `(ZERO 만족..)` ← dialog 에 ZERO X → `(0점..)` `(빵점..)` `(만족 없@음..)`
- explain `(turning point ON)` ← dialog 에 turning point X → `(전환점..)` `(분기 ON)` `(흐름 바뀜..)`

면제 (motif library 시그니처):
- `(X ON)` `(X OFF)` 의 `ON` / `OFF` 2자 — 시그니처 motif, `[A-Za-z]{3,}` 룰 자체 제외 (자동 면제)
- `(?????)` `(ㄷㄷ..)` `(ㅠ..)` 등 부호 only / 한글 자음 — 영어 X 자동 면제

사유: 본 채널은 *한국어 토크* 짜집기 쇼츠. 외래어 메타 동사 (reveal/escalation/turning point) 가 자주 등장하면 시청자 입장 *외국물 먹은 톤* 으로 어색. dialog/source 등장한 영어 (실명·로고·MBTI·워드플레이 일부) 는 자연 → 영어 자체 enumeration 룰 X, dialog/source grep 으로 정합 판단 (어휘 enumeration X 원칙 유지).
...
explain 라인 수는 *맥락이 결정*. 영상 길이별 정량 표 / ceiling / 최소 라인 강제 다 폐기.

**원칙**:
- 펀치·셋업·여운 *각 호흡* 마다 explain narration 의 가치가 있으면 박고, 없으면 박지 X
- dialog 자체가 강하면 무 explain OK (0개)
- 단일 인물 close-up + 표정 변화 + 시리즈 영상은 자연스럽게 dense 해짐
- chain narration (예: `(지금..)` `(지금 불륜..)` `(지금 불륜이 문제가..)` ... 같이 한 발화를 작은 단계로 분절 모사) 는 dialog 텍스트 분절 모사라 X (§9.10.2 substring 통과해도 텍스트 분절 모사 자체 회피)

**시그니처 motif 재사용 의무 (hard rule)**: 영상당 최소 1회는 130개 motif pool 에서 시그니처 caption 재사용. 채널 톤 일관성 유지 목적이지 빈도 강제 아님.
...
**Hold 적용 조건: reaction flip 발생 시 Hold 종료**
...
##### 9.10.1 explain 시점 — 스포일러 회피 (mechanical snap 룰 폐기)

> **시각 싱크 보장은 CapCut 후처리로 위임**. 코워크는 *어휘 시점 누설 회피* 만 책임진다. ±200ms mechanical snap / cue duration 동적 룰 모두 폐기 — LLM 이 발화 시점 미세 align 하는 게 신뢰 X.

★★ 작성 원칙: explain 안에 **dialog 발화 어휘** 를 사용할 때, 그 어휘의 dialog 첫 발화 시점보다 explain 이 먼저 등장하면 시청자에 미리 누설 (스포일러).

작성 절차:
1. dialog.srt 를 시간순으로 정독, 각 발화의 핵심 명사·동사·고유명사가 **몇 초** 에 발화되는지 파악
2. explain 안에 **dialog 발화 어휘를 사용하는 경우**:
 - 그 어휘의 dialog **첫 발화 시점** 을 찾고
 - explain.start 를 그 발화 시점과 **동시 또는 직후** 로 잡는다 (먼저 누설 X)
3. dialog 에 등장하지 않는 어휘 (의태어·X중·X ON·감정 명사·검열·시그니처 motif) 는 시점 룰 무관 — 자유

위반 예시 (어휘 누설 — 시점 어긋남):
- explain `(역@할 카페 사장)` 13.5s ← dialog "역할" 첫 발화 25.5s = +12초 선행 ❌
...
`(?)` `(?????)` 같은 의문 표기 (Family 6 부호 only motif) 는 그 시점 dialog 컨텍스트가 *되묻기 / 놀람 / 말 막힘 / 궁금증 / 침묵* 일 때만 정합. 다음 cue 시점 ±2초 dialog 본문에 다음 중 하나 등장 필요:

- 의문문 종결 어미 (`?` `~?` `~까?` `~어?` `~지?` `~게?`)
- 놀람 / 의문 reaction 어휘 (`엉?`, `어?`, `네?`, `뭐게?`, `진짜?`, `갑자기?`)
- 멈춤 마커 (`...`, `(흠)`, `~~`, 말 끊김)
- MBTI / 이름 / 제목 등 묻는 컨텍스트 (`뭐야`, `누구`, `어디`, `어떻게`)

✅ OK 컨텍스트:
- dialog 의 `뭐게? 제목 뭐야?` + explain `(?????)` (제목 묻는 상황)
- dialog 의 `네?ㅋㅋㅋㅋ` + explain `(?????)` (놀람)
- dialog 의 `위험하게 물건을 놓는다거나 아니면 / 아~ 막` + explain `(?????)` (말 막힘)

❌ 컨텍스트 위반:
- dialog 의 `저 친구 너무 총명하다 / 그렇게 생각했어요` (칭찬) + explain `(?????)` ← 감탄·칭찬 reaction 인데 의문 표기 mismatch
- dialog 의 `ㅋㅋㅋㅋ / 다비치라고 해` (폭소) + explain `(?????)` ← 폭소 reaction 인데 의문 표기 mismatch
- 정정: 칭찬·폭소 자리는 Family 4 (감정명사) 또는 Family 12 (결과형) 으로 교체 — 예 `(인정..)`, `(찐@총명)`, `(딥빡 ㅋㅋ)`
...
> **TV 예능 자막 narration 형 — 가볍고 친근한 톤 + 객관 + 짧은 명사형 + 의성어·의태어 + 모드 토글 (ON/OFF, frame 직관 인지 가능한 모드/상태만) + 검열 시그니처 (@ 비율 0~50% 균형) + Hold 강조 (reaction flip 시 Hold 종료). 시그니처 caption library (130개 motif pool) 영상 간 재사용. dialog 발화 어휘 시점 정합 (스포일러 회피). frame 화면 상황 + dialog reaction 과 3자 정합 (정반대·시점어긋남 X). dialog 와 완전 동일 X. 어휘 enumeration 강제 폐기 — reveal/예고/선언/평가/살벌 동사 모두 frame+reaction 정합이면 사용 OK. (1) 편집·제작 메모성 단일 카테고리만 hard rule.** (@ ceiling 60%, drift 회피)
...
> **정체성 — 짧고 후킹**
>
> 주변 사용자 피드백: "제목이 너무 길다 / 너무 재미없다 / 간결한 훅을 보여줘". 발화 직접 인용 강제 비율 폐기 (옛 룰: 5개 중 3+ 발화 인용). 길이 15~25자 권장 폐기. *짧고 호기심 유발하는 훅* 이 본질.

**길이 룰**:
- **target 5~12자** — 모바일 헤더 한 줄에 시원하게 들어가는 길이
- 5자 이하: 키워드 한방 (`다시 사줘`, `IQ 180`)
- 6~10자: 짧은 훅 (`100억 vs 우정`, `왜 싫었어?`)
- 11~15자: 약간 긴 훅 (`개코보다 민경 랩`)
- 16자+: 두 줄로 (template title_line_count = 2 자동 강제)
- **30자 hard cap** — 그 이상은 hook 효과 X, 시청자 안 읽음

**훅 형식 (다양성 권장 — 강제 비율 X)**:

| 형식 | 예시 | 비고 |
|---|---|---|
| 질문형 | `100억이면 친구 손절?` `왜 싫었어?` `다 알아요?` | 호기심 유발 |
| 단언·충격형 | `유부남의 야ㅋㅋ` `다듀가 발려` `남편 PTSD` | 의외성 |
| 키워드 한방 | `다시 사줘` `IQ 180` `다영 등장` | 짧은 임팩트 |
| 비교형 | `100억 vs 우정` `민경 > 개코` | 대립 구도 |
| 발화 짧은 인용 | `"다시 사줘"` `"개코보다 잘해"` | 인용이 *짧을 때만* — 긴 인용 X |

**훅 원칙**:
- 시청자 클릭 유발 = *호기심* / *공감* / *충격* / *재미* 중 하나 명확히
- 평이한 설명 X (`~의 이유` `~하는 방법` `~의 비결` 같은 추상 메시지)
- *반전 / 의외성 / 누락 정보* 시그널 박기 (단 떡밥 회피 — 영상에서 안 풀리는 호기심 X)
- 인물 호칭 가능 (다영 / 개코 / 타블로) — §9.5 인물 호칭 우선순위 따름

**영상 내용 일치 (soft rule)**:
- 제목 키워드 가능한 한 dialog 안 등장 (강제 50% 매치 폐기 — 너무 빡빡)
- *발화에 전혀 없는 추상 메시지* 만 회피 ("진실" / "본질" / "정의" / "한마디" 같은 메타 해석)
- 훅 키워드는 dialog 핵심 발화 어휘 기반으로 추출 권장

**비교 예시 (옛 → 신)**:

| 옛 제목 (audit 25 영상) | 신 훅 제목 |
|---|---|
| `"개코보다 민경이가 랩 더 잘해"` (18자) | `개코보다 잘해` (7자) |
| `"100억을 준다고 하면 / 평생 쌩깔 수 있다"` (27자) | `100억 vs 우정` (8자) |
| `"지금 소주 한 잔 콜? 이러면 그냥 그때 모이는 건데"` (31자) | `지금 소주 콜?` (7자) |
| `"ENFP 중에 선배님이 최고 학력일걸?"` (23자) | `ENFP 학력 1위` (8자) |
| `"유부남한테 저 톤의 야는 듣자마자 PTSD"` (25자) | `유부남의 야ㅋㅋ` (7자) |
| `"처음엔 싫어? 인사를 너무 해서 싫었대요"` (24자) | `왜 싫었어?` (5자) |
| `"다시 사줘"` (5자) | `다시 사줘` (그대로 OK — 이미 짧음) |
| `"별자리랑 혈액형만 믿어"` (14자) | `별자리만 믿음` (6자) |

**검증 (모드 A / 모드 B self-check)**:

#### 모드 A 시점 — 후보 추천 출력 직전 self-check (잠정 제목)

| 검증 | 기준 |
|---|---|
| 길이 ≤ 30자 (hard cap) | 모든 제목안 |
| 길이 target 5~12자 (1줄) / 13~30자 (2줄) | 권장 |
| 훅 형식 ≥ 1 (질문/단언/키워드/비교/짧은 인용) | hard |
| 추상 메시지 단어 단독 사용 X | hard |
| 떡밥 패턴 회피 | hard |

#### 모드 B 시점 — 편집점 작성 후 (최종 제목 확정)

dialog 본문 기준 *제목 키워드 등장 권장* (soft, 강제 50% 매치 폐기). 미달 시 다른 짧은 훅으로 교체 검토.

**추상 메시지 단어 단독 사용 회피 리스트**:
- `이유` / `방법` / `본질` / `정의` / `한마디` / `진실` / `포인트` / `비결` / `원리` — 단독 단어 X (예: `~의 이유` 형식 회피)

**떡밥 회피**:
- "절대 풀면 안 되는 ~ 사건", "다음 EP 마지막에 풀어요" 류 hard 금지
- 한국 쇼츠 트렌드 = *자기 완결적* 한 컷 (셋업~펀치~여운). 떡밥은 시청자 frustration
...
```json
{
 "schema_version": "1.2",
 "source": {
 "video_path": "<★ macOS 절대 경로 hard rule — `/Users/joel/showdon/yejjas/<채널>/원본/<basename>.mp4`. sandbox path (`/sessions/.../mnt/...`) / URL (`https://...`) / tilde (`~/...`) 박지 X. 코워크가 자기 sandbox mount path 박으면 macOS .app 빌드 시 못 찾음. backend safety net 자동 변환 있지만 처음부터 정확한 경로 박는 게 정답>",
 "analysis_dir": "<★ macOS 절대 경로 — `/Users/joel/showdon/yejjas/<채널>/분석`. video_path 와 같은 규칙>"
 },
 "candidate": {
 "id": 0,
 ...
 "template": {
 "title_text": "<titles[0].text — target 5~12자 (간결 훅). len(text without quotes) ≥ 16 이면 자연 어절 경계에 \\n 강제 삽입. 30자 hard cap>",
 "title_line_count": "<len(title_text 큰따옴표 제거 후) < 16 면 1, ≥ 16 면 2 (강제). LLM 임의 1 설정 hard 금지>"
 },
 "footer": {
...
- `reason`: `"current_speaker"` | `"next_speaker"` | `"two_shot"` | `"reaction"` (★ v1.10.8 — `"hide_caption"` 폐기, backend 자동 bbox-mask)
...
| **8 강화** (Family 분포 + @ ceiling + 시점/완전동일 + 정합 + Family 2 + 영어 외부) | **explain Family 분포 자동 측정 — TV 예능 자막 narration 형 검증** | 자동 측정 + 보고: 1) Family 1~12 분포 % / 2) `~는데/~인데` 어미 등장 회수 (목표 0) / 3) 길이 분포 / 4) @ 검열 비율 (0~50% 권장) / 5) Hold 회수 / 6) 시그니처 motif 풀 재사용 회수 / 7) 스포일러 카운트 (explain 안 의미 토큰의 dialog 첫 발화 시점 vs explain.start) / 8) dialog ↔ explain 완전 동일 카운트 (공백/검열/부호 제거 후) / 9) 편집·제작 메모성 카운트 (`인서트 따주세요` 등) / 10) §9.10.3 frame+reaction 정합 cue별 검증 / 11) Family 2 X ON/OFF 의 X 가 frame 직관 인지 가능한 모드인지 / 12) 영어 단어 (3자+) cue별 dialog/cast/OCR/source grep (등장 X 면 위반) | (a) `~는데/~인데` 0 hard rule (c) Family 1/3/4/6 합산 70%+ (d) 길이 1~5자 75%+ (e) @ 비율 < 60% hard rule (50%+ warning) (f) Hold ≥ 1회 (g) 시그니처 motif 재사용 ≥ 1회 (i) 스포일러 0건 (§9.10.1 Family 9 면제) (j) dialog ↔ explain 완전 동일 0건 (§9.10.2) (k) 편집·제작 메모성 0건 (§9.1 (1)) (k-bis) §9.10.3 정합 위반 0건 — cue별 frame+reaction+explain 3자 정합. 어휘 enumeration 폐기, 상황 정합으로 판단 (k-tris) 영어 외부 단어 0건 — cue 안 `[A-Za-z]{3,}` 가 dialog/cast/OCR/source 등장 X 면 위반 (m) Family 2 X ON/OFF 의 X 가 frame 직관 인지 가능한 모드 |
...
| **10** | **후보 재미 검증 — 펀치 명확 + 시청자 반응 유발 + 펀치 timing ∈ shorts 범위** | 후보 dialog 통독 + 핵심 frame 정독 후 (a) **펀치 명확** = 셋업~빌드업~펀치~여운 호흡 사이클이 후보 안에 있고 펀치 지점이 정확히 식별되는가 / (b) **시청자 반응 유발** = 펀치가 의외성·웃음·공감·놀람 중 하나 이상을 명확히 유발하는가 / **(c) 펀치 timing 검증** = `selection_reason` 의 펀치 발화 시점 (source 시각) 이 `shorts.start_s ≤ punch_t ≤ shorts.end_s` 안에 있는지 | (a) 펀치 지점 1개 이상 명시 가능 (b) 펀치가 의외성·웃음·공감·놀람 중 ≥1 명확 유발 (c) 펀치 timing ∈ shorts 범위 hard rule. 위반 시 (a)(b) → 후보 reject / (c) → `shorts.end_s` 연장하여 펀치 + 1.5초 여운 포함 (180초 cap 안에서) |
| **11** | **영상 전후 맥락 짤림 방지 + 제목 연관 검증** | (a) **시작 자연 진입** = 후보 `start_s` 가 인물 첫 발화 자연 진입 (이전 sub_cut 한복판 진입 X) / (b) **끝 자연 종료** = 후보 `end_s` 가 펀치·여운 자연 종료 (다음 토픽 한복판 자르기 X) / (c) **떡밥 회피** = 후보 안에 던져진 떡밥이 후보 안에서 풀림 / **(e) 제목 연관 검증** = 후보 안 모든 dialog cue 가 *제목 발화 (`key_phrase`) 와 같은 토픽* 인지. 다른 토픽 발화 (이전 코너 마무리 / 무관 잡담 / 토픽 전환 후 빌드업) 들어가있으면 trim 의무. 인물 대화 시작~끝 *전체* 가 후보면 위반 — 제목 발화 중심으로 좁히기. *★ v1.10.7 (d) 회상 클립 룰 폐기 — face_cluster fine-grained 라 false positive 84%, cast_list cluster 매핑 인프라 (v1.10.9) 채워진 후 재활성화 검토* | (a)·(b)·(c)·(e) 모두 통과. (e) 위반 시 §9.3 boundary 재계산: 제목 발화 위치 ± 직접 셋업 1~2 cue ± 즉시 여운 1~2 cue 로 trim |
...
- **8 미달** (갱신 — (k)(l)(m)(n) 추가)
 ...
 - (k) 편집·제작 메모성 (`인서트 따주세요` `이 컷 살려주세요` `다음 장면에..`) 발견 → §9.1 변환: 그 시점 frame 상황 narration 으로 교체
 - (k-bis) §9.10.3 frame+reaction 정합 위반 발견 → cue 별 3자 정합 재작성. 예: 칭찬 컨텍스트에 `(?????)` (감탄 ≠ 의문) → `(인정..)`. 발화자 화면 X 인데 `(우정 결론 ON)` → 발화자 등장 시점으로 cue.start 이동
 - (l) 영어 외부 단어 발견 → §9.1 변환 가이드 참조 (한국어 변환)
...
- **10 미달**
 ...
 - (c) 펀치 timing ∉ shorts → `shorts.end_s` 를 펀치 + 1.5초 여운 포함하도록 연장 (180초 cap 안에서). 또는 펀치 직전 셋업 일부 잘라서 `start_s` 미루기. selection_reason 도 새 timing 반영
...
- **11 미달**
 ...
 - (c) 떡밥 회피 미달 (후보 안에서 안 풀린 떡밥 존재) → 떡밥 풀리는 시점까지 `end_s` 연장 또는 떡밥 자체 회피. 풀 수 없으면 후보 reject
 - (d) ★ v1.10.7 폐기 — face_cluster consolidation (cast_list cluster→name 매핑 v1.10.9) 이전엔 룰 noise
 - **(e) 제목 무관 발화 포함** → §9.3 boundary 재계산. 제목 발화 위치 식별 → 직접 셋업 1~2 cue + 즉시 여운 1~2 cue 로 trim. 후보 길이 짧아져도 OK (15~20초 가능). 인물 대화 시작~끝 전체 포함 패턴 hard 회피
...
✅ §13.5 통합 sync 재검증 통과 (11 항목 / 강화)
 ...
 8. explain Family 분포 + 카테고리 — Family 1/3/4/6 합산 X% (70%+) / `~는데/~인데` 어미 V개 (0) / 길이 1~5자 Y% (75%+) / @ 비율 R% (cap 60%, 0~50% 권장) / Hold N회 / 시그니처 motif 재사용 M회 / 라인 수 K개 / 스포일러 0건 / dialog ↔ explain 완전 동일 0건 / 편집·제작 메모성 0건 / §9.10.3 frame+reaction 정합 cue별 통과 / 영어 외부 단어 0건 / Family 2 X ON 유효 모드 검증 통과
...
## 후보 길이 룰
- 후보 길이 **≤ 180초 hard cap** (그 이상이면 쇼츠 흐름 깨짐)
- 하한 / sweet spot 강제 X — 한 편집점의 *맥락 (셋업~빌드업~펀치~여운)* 자연 시작·끝 기준
- 짧은 펀치 케이스 (15s) / 긴 빌드업 케이스 (150s) 둘 다 OK. 맥락이 결정
# §7. 자막 위치 컨벤션 (캔버스 기준)
...
## §9.3 — 후보 boundary 룰 (제목 발화 중심)

> **정체성 — 제목 발화 중심**
>
> 후보 boundary = **제목으로 선정한 핵심 발화 (`key_phrase`)** 직접 연관 부분만. 인물들의 대화 흐름 시작~끝 *전체* 가 들어가면 길어지고 *제목과 무관한 내용* 까지 포함되어 시청자 입장 지루.
>
> 즉 후보 = "제목 발화 ± 직접 셋업 + 즉시 여운". 그 너머 (다른 토픽 전환 / 일반 대화 / 무관한 셋업) = boundary 밖.

**Boundary 결정 알고리즘**:

1. **제목 발화 위치 식별** — `key_phrase` 가 등장하는 dialog cue 시점 찾기 (= **펀치 시점**)
2. **직접 셋업** — 제목 발화 *이해에 필수* 한 직전 dialog cue 만 (보통 **1~2 cue, 5~10초 이내**). 제목 발화의 *조건·전제·질문* 인 발화만 포함. 그 이전은 다른 토픽 → 제외
3. **즉시 여운** — 제목 발화 직후 *reaction 한 박자* (보통 **1~2 cue, 3~5초 이내**). 다음 토픽으로 넘어가는 발화 등장하면 거기서 cut
4. **180초 cap** — 그래도 cap 안에서 (대부분 후보는 자연스럽게 15~45초 사이로 떨어짐)

**제목 연관 검증 (★ hard rule)**:

후보 안 모든 dialog cue 가 *제목 발화와 같은 토픽* 인지 점검. 다음 케이스는 boundary 밖 — trim 의무:

- ❌ 제목 발화 *훨씬 전* 의 다른 토픽 (예: 제목이 "아이폰 17 다시 사줘" 인데 후보 시작이 *이전 코너 마무리* 또는 *날씨 이야기*)
- ❌ 제목 발화 *후* 다른 토픽 전환 발화 (예: 제목이 "아이폰 17 다시 사줘" 인데 후보 끝부분에 *유튜브 잡탕* 토픽 시작)
- ❌ 셋업 5~10초 *넘는* 빌드업 (제목 발화 이해에 필수 X 인 잡담)
- ❌ 여운 3~5초 *넘는* 후속 대화 (다음 토픽 한복판 가는 패턴)

**boundary 정량 신호 (제목 발화 시점 ±)**:

| # | 신호 | 입력 데이터 | 시점 |
|---|---|---|---|
| 1 | 마지막 발화 후 **0.5s+ silence** | `stt.segments[i].end ~ segments[i+1].start gap ≥ 0.5s` | start/end 양쪽 |
| 2 | scene_cuts 직후 **컷 전환** | `scene_cuts.cut_times` 와 ±0.3s | start/end 양쪽 |
| 3 | 토픽 전환 (★ NEW v1.10.4) | dialog 본문의 *주어/대상이 바뀌는* 발화 (예: "근데 그건 그렇고", "아 그래서 어쩜요", "다음 질문은") | start (이전 토픽 cut) / end (다음 토픽 cut) |
| 4 | face_clusters dominant cluster_id 변화 + 토픽 전환 | 화자 전환 + 발화 컨텍스트 변화 | start/end |

**예시**:
```
원본 dialog timeline:
00:50 "어제 비 왔잖아 진짜 추웠어"            ← 무관 토픽
00:54 "근데 그건 그렇고 아이폰 17 새로 산 거"  ← 토픽 전환 (신호 3) + 셋업 시작
00:57 "친구가 떨어뜨려서 액정 박살"            ← 직접 셋업
01:00 "다시 사줘"                            ← 제목 발화 (펀치)
01:02 "그렇게 그게 끝이에요?"                  ← 즉시 reaction (여운)
01:04 "어 다시 사줘"                          ← reaction 마무리
01:06 "유튜브 얘기로 넘어가서"                ← 다음 토픽 (신호 3 — cut)

→ 후보 boundary = 00:54 ~ 01:06 (12초)
   셋업 6초 + 펀치 2초 + 여운 4초
```

**길이 가이드 (참고만 — 강제 X)**:
- 짧은 한방 한 마디 펀치: 10~20초
- 셋업·펀치·짧은 여운: 20~40초
- 빌드업 긴 펀치: 40~60초
- compilation 시리즈: 60~120초
- 180초 cap

selection_reason 에 **제목 발화 위치 + 셋업 cue 수 + 여운 cue 수** 명시 의무 — 예: "셋업 2 cue (00:54~00:59) + 펀치 (01:00 '다시 사줘') + 여운 2 cue (01:02~01:06) = 12s"
...
**2. web_search** — 인물 실명 + EP 맥락 (채널·시리즈) 확인. 출연진·게스트 모두 (이미 0번에서 했으면 재사용)
**3. 위 우선순위 표 적용**
**4. 일관성** — 같은 인물 = 한 영상 안에서 항상 같은 호칭 (혼용 X). 첫 호칭 결정 후 끝까지 그것만.

**5. explain 인물명 grep 검증 (★ hard rule — 환각 차단)**

explain.srt 안 *2자 이상 한국어 고유명사 후보* (예: `다영`, `혜리`, `예원`, `종이`, `매질`) 자동 추출 후 다음 중 *어디 하나* 에 등장해야 PASS:
- `cast_list.json` 의 name 필드
- `dialog.srt` 본문 (해당 sub_cut 또는 그 직전 sub_cut)
- `source_meta.json` 의 description
- 영상 안 OCR (`ocr_local.json` / `ocr_candidates.json`) 에 자막 / 자체 노출

어느 곳에도 없으면 **환각 의심** → fallback ("언니" / "옆 사람" / "그 친구" / "게스트") 으로 교체 or 그 라인 삭제.

위반 예: `(다영 언니 매질..)` — "다영" 이 cast list (다비치 = 이해리 + 강민경) 에도 dialog 에도 없음. fallback `(혜리 언니 매질..)` 또는 `(언니 매질..)`.
...
- **분포 (영상 3등분)** — 영상을 0~33% / 33~66% / 66~100% 세 영역으로 나눠 explain 라인이 한 영역에 몰리지 않게. 영상 시작부에만 몰리면 후반부 정리 부족 — 마지막 펀치/여운 강조 explain 1개+ 권장
- **영상 내 explain ratio max 50% 강제 트림** — 한 영상의 explain ratio 가 50% 초과면 explain 라인 줄이기 의무

**원칙**:
- *최대 갯수* 강제 X / *최소 갯수* 강제 X — 맥락이 결정. 영상 길이별 정량 표 폐기.
- 펀치·셋업·여운 *각 호흡* 마다 explain narration 의 가치가 있으면 박고, 없으면 박지 X
- dialog 자체가 강하면 무 explain OK (0개)
- 단일 인물 close-up + 표정 변화 + 시리즈 영상은 자연스럽게 dense 해짐
...
| C5 | §10 **제목 간결 훅** vs **dialog 키워드 등장 권장** | **훅 강도 우선** (짧고 후킹) — dialog 키워드 매치는 soft. 단 추상 메시지 단독 사용 hard 금지 | 주변 사용자 피드백 — 길고 재미없는 제목 회피 |
