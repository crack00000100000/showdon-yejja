...
#### 4. Wide shot (3+명) hide_caption — dialog 일치 자체 자막만

face_clusters frames 에서 **face 개수 ≥ 3** 인 시간대 비중이 50%+ 인 wide shot 자체는 hide_caption 트리거 X. 그 시간대에 *원본 영상의 dialog 일치 자체 자막* 이 검출돼야 hide_caption.

판정 절차:
1. wide shot 판정 (face ≥ 3, 50%+) — 조건 1
2. 그 sub_cut 시간대의 `ocr_candidates[]` 또는 `ocr_local.json` 에서 화면 하단 자막 텍스트 추출
3. 그 OCR 텍스트가 해당 시간대 dialog cue 텍스트와 **substring 일치** (공백/부호 제거 후) 여부 확인 — 조건 2
4. 둘 다 충족 시만 `focus_box y=0, h=0.85` 강제 + reason = `"hide_caption"`

원칙: dialog 와 일치하는 자체 자막만 회피. 워터마크 / 채널 로고 / 간판 / 예능 자체 자막 (dialog 와 substring 일치 X) 은 노출 OK.

dialog 와 무관한 자체 자막 (예: 한정판매 간판, 인물 옷 텍스트, 채널 워터마크 `epikase`) 은 노출 OK — 일반 시청자가 영상 컨텐츠로 자연스럽게 인지하는 텍스트.

#### 5. 얼굴 정보 없으면 focus_box null (폴백 사용)

#### 6. 자막 가리기 — dialog 일치 자체 자막만

sub_cut 시간대 ocr_candidates frame 의 화면 하단 자막 텍스트가 *해당 시간대 dialog cue 텍스트와 substring 일치* (공백/부호/검열 기호 제거 후) 인 경우만 → `y=0`, `h≤0.85`. reason = `"hide_caption"`.

substring 일치 X 인 자체 자막 (워터마크 / 간판 / 일반 OCR 텍스트) 은 가리지 X — 노출 OK.

예시:
- 회상 클립 자체 자막 "때려 죽어도 항상 둘씩 붙어다닐거야" + dialog "내 생각에는 우정에 대한 답은 다비치처럼만 살면 돼" → substring 일치 X → 가리지 X.
- 1억 코너 화면 자체 자막 "1억을 준다고 하면 / 평생 못 본 척할 수 있다" + dialog 같은 발화 → substring 일치 → hide_caption.
- 우상단 워터마크 `epikase` → dialog 와 무관 → 노출 OK.
