...
- [ ] **§13.5 10·11 펀치 timing + 회상 회피** — (10c) `selection_reason` 의 펀치 시점이 `shorts.start_s ~ shorts.end_s` 안. 위반 시 `end_s` 연장 (180초 cap 안에서) / (11d) 후보 `start_s` ±5초 face cluster 가 발화자 cluster 와 일치. 회상/cutaway 인서트 발견 시 발화자 등장 시점으로 `start_s` 재선정
...
예시:
- 회상 클립 자체 자막 "때려 죽어도 항상 둘씩 붙어다닐거야" + dialog "내 생각에는 우정에 대한 답은 다비치처럼만 살면 돼" → substring 일치 X → 가리지 X. 다만 §13.5 11항 (d) 회상 클립 자리 재선정으로 별개 차원에서 회피.
...
- 폭로 시점에 정확히 `(헤이터박터 reveal!!)` (시청자 reveal 시점과 동시 — 누설 X)

❌ frame + reaction 과 반대 방향 / 시점 어긋남 / 미리 누설:
- 발화자가 이름 못 꺼낸 시점에 `(다영 등장 예고..)` (frame 에 다영 없음 + 시청자 reveal 미리 누설)
...
★★ 작성 원칙: explain 안에 **dialog 발화 어휘** 를 사용할 때, 그 어휘의 dialog 첫 발화 시점보다 explain 이 먼저 등장하면 시청자 reveal 미리 누설.
...
| **11** | **영상 전후 맥락 짤림 방지 + 회상 클립 자리 재선정 + 제목 연관 검증** | (a) **시작 자연 진입** = 후보 `start_s` 가 인물 첫 발화 자연 진입 (이전 sub_cut 한복판 진입 X) / (b) **끝 자연 종료** = 후보 `end_s` 가 펀치·여운 자연 종료 (다음 토픽 한복판 자르기 X) / (c) **떡밥 회피** = 후보 안에 던져진 떡밥이 후보 안에서 풀림 / (d) **회상 클립 자리 재선정** = 후보 `start_s` ±5초 face_clusters dominant cluster 가 발화자 cluster 와 일치. 불일치 = 회상/cutaway 인서트 → 발화자 등장 시점으로 `start_s` 재선정 / **(e) 제목 연관 검증 (★ NEW v1.10.4)** = 후보 안 모든 dialog cue 가 *제목 발화 (`key_phrase`) 와 같은 토픽* 인지. 다른 토픽 발화 (이전 코너 마무리 / 무관 잡담 / 토픽 전환 후 빌드업) 들어가있으면 trim 의무. 인물 대화 시작~끝 *전체* 가 후보면 위반 — 제목 발화 중심으로 좁히기 | (a)·(b)·(c)·(d)·(e) 모두 통과. (e) 위반 시 §9.3 boundary 재계산: 제목 발화 위치 ± 직접 셋업 1~2 cue ± 즉시 여운 1~2 cue 로 trim |
...
- **11 미달**
 ...
 - (d) 회상 클립 / cutaway 인서트 → `face_clusters.frames[]` 에서 발화자 cluster_id 등장 시점으로 `start_s` 재설정
